...
CREATE TABLE Actividades (
  idActividad CHAR(4) PRIMARY KEY,
  actividad   VARCHAR(260)
);

GRANT SELECT ON Actividades TO gestion;
INSERT INTO Actividades(idActividad, actividad) 
VALUES
...
CREATE TABLE Clientes (
  ...
  CONSTRAINT Clientes_idCliente_PK PRIMARY KEY (idCliente),
  CONSTRAINT Clientes_tipo_CK CHECK(tipo IN ('E', 'P'))
  -- UNIQUE KEY Clientes_NIF_UK (NIF)
);

CREATE UNIQUE INDEX Clientes_NIF_UK ON Clientes (NIF);
-- ALTER TABLE Clientes DROP INDEX Clientes_NIF_UK

GRANT SELECT, INSERT, DELETE ON Clientes TO gestion;
GRANT UPDATE (NIF, nombre, direccion, poblacion, provincia, 
              pais, codigoPostal, fechaModificacion)
      ON Clientes TO gestion;

CREATE TABLE Personas (
  idCliente         INT NOT NULL,
  apellidos         VARCHAR(120) NOT NULL,
  CONSTRAINT Personas_idCliente_PK PRIMARY KEY (idCliente),
  CONSTRAINT Personas_idCliente_FK FOREIGN KEY (idCliente) REFERENCES Clientes(idCliente)
);

GRANT SELECT, INSERT, UPDATE, DELETE ON Personas TO gestion;

CREATE TABLE Empresas (
  idCliente         INT NOT NULL,
  actividad         CHAR(4) NOT NULL,
  gerente           INT NOT NULL,
  empresaMatriz     INT DEFAULT NULL,
  CONSTRAINT Empresas_idCliente_PK PRIMARY KEY (idCliente),
  CONSTRAINT Empresas_idCliente_FK FOREIGN KEY (idCliente) REFERENCES Clientes(idCliente),
  CONSTRAINT Empresas_actividad_FK FOREIGN KEY (actividad) REFERENCES Actividades(idActividad),
  CONSTRAINT Empresas_gerente_FK FOREIGN KEY (gerente) REFERENCES Personas(idCliente),
  CONSTRAINT Empresas_empresaMatriz_FK FOREIGN KEY (empresaMatriz) REFERENCES Empresas(idCliente)
);

GRANT SELECT, INSERT, UPDATE, DELETE ON Empresas TO gestion;

CREATE VIEW allPersonas AS
  SELECT c.idCliente, c.NIF, c.nombre, p.apellidos, c.direccion, c.poblacion, 
         c.provincia, c.codigoPostal, c.fechaAlta 
    FROM clientes c, personas p 
   WHERE c.idCliente = p.idCliente;

GRANT SELECT ON allPersonas TO gestion;

CREATE VIEW allEmpresas AS
  SELECT c.idCliente, c.nif, c.nombre, c.direccion, c.poblacion, c.provincia, 
         c.codigoPostal, c.fechaAlta, e.gerente, e.empresaMatriz, a.actividad 
    FROM clientes c, empresas e, actividades a 
   WHERE c.idCliente = e.idCliente AND e.actividad = a.idActividad;

GRANT SELECT ON allEmpresas TO gestion;


INSERT INTO Clientes(NIF, nombre, direccion, poblacion, provincia, pais,
                     codigoPostal, fechaAlta, tipo)
VALUES
 ('021222980', 'HENKEL AG & Co.', '', 'Düsseldorf', '', 'Alemania', '40191', '2015-01-03', 'E'),
 ('439281933', 'Schwarzkopf', 'Pol. Canalla p12', 'Malaga', 'Malaga', 'España', '34301', '2015-02-03', 'E'),
 ('23330101K', 'Marcos', 'Av. Camelias, 4', 'Mislata', 'Valencia', 'España', '46322', '2014-06-20', 'P'),
 ('86945829Z', 'Jorge', 'Milagros, 9', 'La Cañada', 'Valencia', 'España', '46991', '2013-08-10', 'P'),
 ('48295003J', 'Lourdes', 'Calle Larga, 233', 'Torrente', 'Valencia', 'España', '46229', '2014-01-24', 'P'),
 ('J9494300M', 'TEROSON', 'Pol. Rojo, p32', 'Valencia', 'Valencia', 'España', '46014', '2015-03-01', 'E');

INSERT INTO Personas(idCliente, apellidos)
  SELECT idCliente, 'González Churro' FROM Clientes WHERE NIF = '23330101K';

INSERT INTO Personas(idCliente, apellidos)
  SELECT idCliente, 'Paraná Mayor' FROM Clientes WHERE NIF = '86945829Z';

INSERT INTO Personas(idCliente, apellidos)
  SELECT idCliente, 'Lancho Novo' FROM Clientes WHERE NIF = '48295003J';


INSERT INTO Empresas(idCliente, actividad, gerente, empresaMatriz)
  SELECT c.idCliente, '2041', g.idCliente, NULL 
    FROM Clientes c, Clientes g
   WHERE c.NIF = '021222980' AND g.NIF = '48295003J';

INSERT INTO Empresas(idCliente, actividad, gerente, empresaMatriz)
  SELECT c.idCliente, '2042', g.idCliente, m.idCliente 
    FROM Clientes c, Clientes g, Clientes m
   WHERE c.NIF = '439281933' AND g.NIF = '23330101K' AND m.NIF = '021222980';

INSERT INTO Empresas(idCliente, actividad, gerente, empresaMatriz)
  SELECT c.idCliente, '4752', g.idCliente, m.idCliente 
    FROM Clientes c, Clientes g, Clientes m
   WHERE c.NIF = 'J9494300M' AND g.NIF = '86945829Z' AND m.NIF = '021222980';
